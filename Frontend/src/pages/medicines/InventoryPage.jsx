...
import { Search, Boxes, AlertTriangle, PackageX, Clock, Package } from "lucide-react";
...
export default function InventoryPage() {
  ...
  const loadProducts = useCallback(async (q = "") => {
    ...
    setError("");
    try {
      const data = await fetchProducts({ search: q || undefined });
      setProducts(data || []);
    } catch (err) {
  ...
  const counts = useMemo(() => {
    ...
    return { total: products.length, low, out, expired, near };
  }, [products, statusOf]);

  return (
    <div className="flex flex-col gap-4">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <StatCard label="Total Products" value={counts.total.toLocaleString()} sub="In inventory" icon={Package} tone="blue" />
      ...
      <Card>
        <CardHeader
          title="Stock Status"
          subtitle="Monitor quantities, reorder levels and expiry"
          action={
        ...
        {loading ? (
          ...
          <EmptyState icon={Boxes} title="No products match" subtitle="Try another search or stock status." />
        ) : (
                  ...
                  return (
                    <tr key={p.id} className="hover:bg-slate-50/60 transition-colors">
                      <td className="px-4 py-3">
                        <span className="font-semibold text-[13px] text-slate-900">{p.name}</span>
                        <span className="text-[12px] text-slate-400 block font-normal">{p.brand || p.barcode}</span>
                      </td>
                      <td className="px-3 py-3">
                        ...
                        <div className="text-[11px] text-slate-400 font-normal text-right mt-0.5">
                          ≡ {formatEquivalents(stock, p)}
                        </div>
                      </td>
                      <td className="px-3 py-3 text-slate-500 font-normal">{reorder.toLocaleString()}</td>
                      ...
                      <td className="px-4 py-3">
                        <StatusBadge
                          status={st === "out" ? "Out of stock" : st === "low" ? "Low" : st === "expired" ? "Expired" : "In stock"}
                        />
                      </td>
                    </tr>
                  );
