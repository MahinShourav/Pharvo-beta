      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="h-[76px] flex-shrink-0 bg-white border-b border-slate-100 flex items-center justify-between px-4 sm:px-8 z-10">
          ...
          <div className="flex items-center gap-3.5">
            {/* Search bar */}
            <div className="relative w-56 sm:w-72 hidden sm:block">
              <Search size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400" />
              <input
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                placeholder="Search medicines, orders..."
                className="w-full pl-10 pr-4 py-2.5 text-sm font-normal rounded-lg border border-slate-200 outline-none focus:border-slate-400 focus:bg-white bg-slate-50/50 transition-all duration-100"
              />
            </div>
