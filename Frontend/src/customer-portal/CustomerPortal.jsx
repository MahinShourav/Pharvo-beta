...
import { Bell, Phone, ReceiptText, Repeat, Sparkles, Truck } from "lucide-react";
...
export default function CustomerPortal() {
  ...
  /* Active orders: no customer-safe orders endpoint exists in scope
   * (portal reads completed counter receipts via GET /api/sales/ only),
   * so there is nothing to list — the section hides entirely per spec.
   * "Track order" below scrolls to purchase history instead. */
  ...
  const quickActions = [
    ...
    },
    {
      icon: Truck,
      label: "Track order",
      onClick: () => scrollToSection(orders.length > 0 ? "portal-orders" : "portal-purchases"),
    },
    {
      icon: Phone,
        ...
        <main className="flex-1 w-full max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-28 lg:pb-12">
          ...
          {status === "ready" && (
            <>
              ...
              {/* Active orders — hidden entirely when there are none */}
              {orders.length > 0 && (
                <section id="portal-orders" aria-label="Active orders">
                  <SectionHead title="Active orders" subtitle={`${orders.length} in progress`} />
                  <div className="flex gap-4 overflow-x-auto snap-x portal-no-scrollbar pb-1 -mx-4 px-4 sm:mx-0 sm:px-0">
                    {orders.map((order) => (
                      <PortalCard key={order.id} className="min-w-[270px] snap-start p-5">
                        <div className="flex items-center justify-between gap-2">
                          <p className="text-sm font-semibold text-[var(--portal-ink)] truncate">#{order.id}</p>
                          <PortalPill tone="warning">{order.status}</PortalPill>
                        </div>
                        <p className="text-[13px] font-normal text-[var(--portal-ink-muted)] mt-1.5">
                          {formatMoney(order.amount)} · {timeAgo(order.at)}
                        </p>
                      </PortalCard>
                    ))}
                  </div>
